...
@Converter
public class AsyncTaskTypeConverter
        implements AttributeConverter<AsyncTaskType, String> {

    private static final Map<String, AsyncTaskType> TYPES =
    ...
    public static void register(AsyncTaskType... types) {
        ...
        for (AsyncTaskType type : types) {
            ...
            AsyncTaskType previous = TYPES.putIfAbsent(
                    required.value(), required);
            if (previous != null && !previous.equals(required)) {
                throw new IllegalStateException(
                        "Async task type is already registered: "
                                + required.value());
            }
        }
    }
    ...
    private record UnknownAsyncTaskType(String value)
