...
public class AsyncTask extends AuditedEntity<Long> {
    ...
    @Id
    @GeneratedValue(
            generator = "async_task_id_generator",
    ...
    public void markProcessing(
            String owner, OffsetDateTime leaseExpiresAt) {
    ...
    public void markProgress(Map<String, Object> payload) {
        ...
        Map<String, Object> progress = progressPayload == null
                ? new LinkedHashMap<>()
                : new LinkedHashMap<>(progressPayload);
    ...
    public void deferCompletion(Map<String, Object> payload) {
        ...
        long expected = numberValue(
                progress.get("work_units_expected"), -1L);
        long completed = numberValue(
                progress.get("work_units_completed"), 0L);
    ...
    public void markFailed(
        ...
        if (attemptCount < maxAttempts) {
            Duration delay = retryDelay == null
                    ? Duration.ofSeconds((long) (30 * Math.pow(
                            4, Math.max(0, attemptCount - 1))))
                    : retryDelay;
    ...
    public boolean processingLeaseExpired(OffsetDateTime timestamp) {
        return status == AsyncTaskStatus.PROCESSING
                && (processingLeaseExpiresAt == null
                        || !processingLeaseExpiresAt.isAfter(timestamp));
    }
    ...
    private void requireActive(String transition) {
        if (status != AsyncTaskStatus.ACCEPTED
                && status != AsyncTaskStatus.PROCESSING) {
            throw new IllegalStateException(
                    "Cannot " + transition + " task in state " + status);
        }
    }

    private void requireStatus(
            AsyncTaskStatus required, String transition) {
        if (status != required) {
            throw new IllegalStateException(
                    "Cannot " + transition + " task in state " + status
