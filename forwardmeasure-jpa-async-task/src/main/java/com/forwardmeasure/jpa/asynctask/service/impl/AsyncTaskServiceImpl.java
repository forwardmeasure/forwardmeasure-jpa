...
public class AsyncTaskServiceImpl
        extends AuditedEntityServiceImpl<
                AsyncTask, Long, AsyncTaskRepository>
    ...
    public List<AsyncTask> findByResourceAndStatus(
        ...
        return repository().findByResourceAndStatus(
    ...
    public List<AsyncTask> listTasks(
        ...
        return repository().listTasks(
                status, taskType, resourceType, page, pageSize);
    }
    ...
    public List<AsyncTask> listByResourceId(
            ...
            int pageSize) {
        return status == null
                ? repository().listByResourceId(
                        resourceId, page, pageSize)
                : repository().listByResourceIdAndStatus(
                        resourceId, status, page, pageSize);
    }

    @Override
    public long countByResourceId(
            UUID resourceId, AsyncTaskStatus status) {
        return status == null
                ? repository().countByResourceId(resourceId)
                : repository().countByResourceIdAndStatus(
                        resourceId, status);
    }

    @Override
    public AsyncTask createTask(
            AsyncTaskType taskType, UUID resourceId, Actor actor) {
    ...
    public AsyncTask createTask(
        ...
        return createTask(
                taskType, resourceId, actor, idempotencyKey, null);
    }
    ...
    public AsyncTask createTask(
        ...
        AsyncTaskType requiredType = Objects.requireNonNull(
                taskType, "taskType");
        AsyncTask task = AsyncTask.builder()
                ...
                .expiresAt(OffsetDateTime.now(ZoneOffset.UTC)
                        .plusSeconds(requiredType.defaultExpirySeconds()))
                .actor(actor)
                .idempotencyKey(normalize(idempotencyKey))
                .idempotencyFingerprint(
                        normalize(idempotencyFingerprint))
                .build();
    ...
    public int cancelByResource(String resourceType, UUID resourceId) {
        List<AsyncTask> active = repository().findActiveByResource(
    ...
    public Optional<AsyncTask> markProcessing(
        ...
        return update(taskId, task -> task.markProcessing(
                processingOwner, leaseExpiresAt));
    }

    @Override
    public Optional<AsyncTask> markCompleted(
            UUID taskId, Map<String, Object> payload) {
        return update(taskId, task -> task.markCompleted(payload));
    }

    @Override
    public Optional<AsyncTask> markCompletedWithUri(
            UUID taskId, String uri) {
        return update(taskId, task -> task.markCompletedWithUri(uri));
    }

    @Override
    public Optional<AsyncTask> deferCompletion(
            UUID taskId, Map<String, Object> payload) {
        return update(taskId, task -> task.deferCompletion(payload));
    }

    @Override
    public Optional<AsyncTask> markProgress(
            UUID taskId, Map<String, Object> payload) {
    ...
    public Optional<AsyncTask> markFailed(
            ...
            Map<String, Object> errorDetail,
            Duration retryDelay) {
        return update(taskId, task -> task.markFailed(
                errorCode, errorSummary, errorDetail, retryDelay));
    }
    ...
    public boolean extendProcessingLease(
            ...
            String processingOwner,
            OffsetDateTime leaseExpiresAt) {
        return repository().findByUuidForUpdate(taskId)
                .map(task -> {
                    boolean extended = task.extendProcessingLease(
                            processingOwner, leaseExpiresAt);
                    if (extended) {
                        repository().flush();
                    }
    ...
    private Optional<AsyncTask> update(
            UUID taskId, Consumer<AsyncTask> transition) {
        return repository().findByUuidForUpdate(taskId)
                .map(task -> {
