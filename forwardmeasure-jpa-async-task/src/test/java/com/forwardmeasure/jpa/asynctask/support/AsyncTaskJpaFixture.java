...
public final class AsyncTaskJpaFixture implements AutoCloseable {
    ...
    public static AsyncTaskJpaFixture create(
        ...
        if (!migrator.status(tenant).current()) {
            throw new IllegalStateException(
                    "Async-task schema is not current after migration");
        }
        return new AsyncTaskJpaFixture(Persistence.createEntityManagerFactory(
    ...
    public Context context(EntityManager entityManager) {
        ...
        return new Context(
                ...
                actors,
                tasks,
                new AsyncTaskServiceImpl(tasks));
    }
