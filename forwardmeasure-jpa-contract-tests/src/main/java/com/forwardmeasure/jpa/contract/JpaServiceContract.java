...
public final class JpaServiceContract {
    ...
    public static ContractResult verify(
            ActorService actors,
            OwnedEntityService<ContractOwnedEntity, Long> owned) {
        ...
        require(actors.findByIdentity(
                        "service-contract-idp",
                        "service-contract-user").isPresent(),
        ...
        require(actors.findByEmail("service-contract@example.test").size() == 1,
        ...
        require(owned.existsByUuid(entity.getUuid()),
                "Owned service UUID existence failed");
        require(owned.findByOwnerId(actor.getId()).size() == 1,
                "Owned service owner lookup failed");
        require(owned.countByOwnerId(actor.getId()) == 1,
                "Owned service owner count failed");
        ...
        owned.flush();
        require(!owned.existsByUuid(disposable.getUuid()),
                "Service deletion remained visible");
