...
public abstract class AbstractBaseRepository<
  ...
  protected final EntityManager entityManager() {
    if (entityManager == null) {
      throw new IllegalStateException(
          "No persistence context was injected into " + getClass().getName());
    }
    return entityManager;
  }

  /**
   * Connects a repository created by a framework factory to that framework's transaction-scoped
   * entity-manager proxy.
   *
   * <p>Application code never calls this method. Quarkus normally injects the {@link
   * PersistenceContext} field directly; Spring and Micronaut adapter configuration uses this hook
   * when it creates the same common repository class through a framework factory.
   */
  public final void bindPersistenceContext(EntityManager context) {
    ...
    if (entityManager != null && entityManager != required) {
      throw new IllegalStateException(
          "Persistence context is already bound for " + getClass().getName());
    }
  ...
  public final Class<T> entityClass() {
    ...
    if (resolved == null) {
      resolved = (Class<T>) RepositoryTypeResolver.resolveEntityClass(getClass());
  ...
  public T findById(I id, LockModeType lockMode) {
    return entityManager()
        .find(entityClass(), requireId(id), Objects.requireNonNull(lockMode, "lockMode"));
  }
  ...
  public Page<T> page(PageRequest request, JpaSpecification<T> specification) {
    ...
    if (specification != null) {
      dataQuery.where(specification.toPredicate(dataRoot, dataQuery, builder));
    }

    List<SortOrder> sort =
        request.sort().isEmpty()
            ? List.of(new SortOrder(identifierAttribute().getName(), SortDirection.ASCENDING))
            : request.sort();
    dataQuery.orderBy(
        sort.stream()
            .map(
    ...
    if (specification != null) {
      countQuery.where(specification.toPredicate(countRoot, countQuery, builder));
    }
    countQuery.select(
        countQuery.isDistinct() ? builder.countDistinct(countRoot) : builder.count(countRoot));
  ...
  private Path<?> resolvePath(Root<T> root, String property) {
    ...
    for (int index = 0; index < segments.length; index++) {
      ...
      if (segment.isBlank()) {
        throw new IllegalArgumentException("Property path contains an empty segment: " + property);
      }
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Unknown persistent attribute " + segment + " in property path " + property, missing);
      }
      path = path.get(segment);
      if (index < segments.length - 1) {
        if (!attribute.isAssociation()) {
          throw new IllegalArgumentException(
              "Non-association attribute " + segment + " cannot be traversed in " + property);
        }
        managedType = entityManager().getMetamodel().managedType(attribute.getJavaType());
      }
    }
    return path;
  }
