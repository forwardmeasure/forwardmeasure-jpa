...
final class RepositoryTypeResolver {
    ...
    static Class<?> resolveEntityClass(Class<?> repositoryClass) {
        ...
        while (current != null && current != Object.class) {
            if (current instanceof ParameterizedType parameterized) {
                ...
                for (int index = 0; index < variables.length; index++) {
                    bindings.put(
                            variables[index],
                            resolve(arguments[index], bindings));
                }
        ...
        throw new IllegalStateException(
                "Unable to resolve the entity type for repository "
                        + repositoryClass.getName());
    }

    private static Type resolve(
            Type value, Map<TypeVariable<?>, Type> bindings) {
        Type resolved = value;
    ...
    private static Class<?> toClass(Type value) {
        ...
        throw new IllegalStateException(
                "Unsupported repository entity type " + value.getTypeName());
    }
}
