...
public class ActorRepository extends AbstractBaseRepository<Actor, Long> {
    ...
    public Optional<Actor> findByIdentity(
        ...
        query.select(actor).where(identityPredicate(
                builder, actor, identityProvider, subjectIdentifier));
        return entityManager().createQuery(query)
                .setMaxResults(1)
                .getResultList()
                .stream()
    ...
    private <V> List<Actor> findBy(
            jakarta.persistence.metamodel.SingularAttribute<Actor, V> attribute,
            V value) {
        CriteriaBuilder builder = criteriaBuilder();
        CriteriaQuery<Actor> query = builder.createQuery(Actor.class);
        Root<Actor> actor = query.from(Actor.class);
    ...
    private static Predicate identityPredicate(
        ...
        Predicate provider = identityProvider == null
                ...
                : builder.equal(
                        actor.get(Actor_.identityProvider), identityProvider);
        return builder.and(
                provider,
                builder.equal(
                        actor.get(Actor_.subjectIdentifier), subjectIdentifier));
    }
}
