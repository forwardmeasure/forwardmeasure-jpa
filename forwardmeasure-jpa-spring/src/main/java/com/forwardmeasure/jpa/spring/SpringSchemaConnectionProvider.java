 ...
 * SpringTenantIdentifierResolver}, which reads it straight off {@code TenantScope}, itself opened
 * with a real {@link TenantDatabase} by whatever resolved the caller's identity) - no lookup needed
 * here, since {@code TenantScope} carries the real routing target, not a bare UUID.
 *
...
public final class SpringSchemaConnectionProvider implements MultiTenantConnectionProvider<String> {
   ...
   *     access. Points at an administrative database (e.g. Postgres's own {@code postgres}
   *     database, or the platform's own tenant-registry database), not any tenant's own database.
   */
  public SpringSchemaConnectionProvider(
      TenantDataSourceRegistry registry, FunctionalSchema schema, DataSource bootstrapDataSource) {
    this.registry = Objects.requireNonNull(registry, "registry");
    this.schema = Objects.requireNonNull(schema, "schema");
    this.bootstrapDataSource = Objects.requireNonNull(bootstrapDataSource, "bootstrapDataSource");
  }
